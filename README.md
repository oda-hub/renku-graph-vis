...
specific nodes along with their child nodes absorbed within. For example, in the animation below, it can be seen that the `Activity` node has some of its child nodes (i.e. inputs and outputs) inside it as well as separate nodes. This functionality is configurable through a dedicated JSON file.
